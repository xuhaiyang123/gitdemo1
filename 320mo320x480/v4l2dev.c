...
int v4l2_enum_fmt(v4l2dev_t *dev)
{
	...
	while ((ret = ioctl(dev->fd, VIDIOC_ENUM_FMT, &fmtd)) == 0) {
		fmtd.index++;
		dbg_info("\nv4l2_enum_fmt PIXFMT = '%c%c%c%c', '%s' \n",
...
int v4l2_read_frame(v4l2dev_t *dev, void **data, int *dlen)
{
	...
	*dlen = 0;
	...
	if (dev->bufs[v4l2buf.index].v4l2buf.length > 0) {
		tick = get_timestamp();	
		//dbg_info("v4l2_read_frame: length= %d\n", dev->bufs[v4l2buf.index].v4l2buf.length);
		*data = malloc(dev->bufs[v4l2buf.index].v4l2buf.length);
		if (*data) {
			*dlen = dev->bufs[v4l2buf.index].v4l2buf.length;
			p = (char *)(dev->bufs[v4l2buf.index].start);
			q = (char *)*data;
			for(i = 0; i < *dlen; i++)
			{
				q[i] = p[i];
			}
			//dbg_info("v4l2_read_frame: dlen= %d\n", *dlen);
		}
	}
	...
	//dbg_info("v4l2_read_frame: VIDIOC_QBUF: ret= %d, dlen= %d\n", ret, *dlen);
	return ret;
}
